        ...
        if (insideTriangle(x, y, v)) { // 如果当前像素在三角形内
          ...
          if (SSAA) {
            ...
            for (float i = 0.25; i < 1.0; i += 0.5) {
              ...
              {
				  auto interpolated_color = alpha * t.color[0] + beta * t.color[1] + gamma * t.color[2];   // 插值计算颜色
				  auto interpolated_normal = alpha * t.normal[0] + beta * t.normal[1] + gamma * t.normal[2]; // 插值计算法线
				  auto interpolated_texcoords = alpha * t.tex_coords[0] + beta * t.tex_coords[1] + gamma * t.tex_coords[2]; // 插值计算纹理坐标
				  auto interpolated_shadingcoords = alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2]; // 插值计算视图空间坐标
