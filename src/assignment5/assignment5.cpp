...
int assignment5(int argc, const char** argv)
{
    ...
    Vector2f st[4] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}}; // 创建四个纹理坐标，分别为(0, 0)，(1, 0)，(1, 1)，(0, 1) 每个顶点对应一个
    uint32_t vertIndex[6] = {0, 1, 3, 1, 2, 3}; // 创建六个顶点索引，分别为0, 1, 3, 1, 2, 3
    // 为什么要创建6个顶点索引? 因为矩形要被分解成2个三角形
    // 索引 `{0, 1, 3}` → 顶点 `{(-5,-3,-6), (5,-3,-6), (-5,-3,-16)}
    // 索引 `{1, 2, 3}` → 顶点 `{(5,-3,-6), (5,-3,-16), (-5,-3,-16)}
